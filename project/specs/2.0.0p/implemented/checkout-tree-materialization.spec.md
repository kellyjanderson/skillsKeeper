# Checkout Tree Materialization Specification
...
- `project/specs/2.0.0p/checkout-lockfile-schema.spec.md` - tree checkout records multiple entries
- `project/specs/2.0.0p/graph-traversal-api.spec.md` - tree expansion uses top-down traversal
...
Count: 1 IWU.
Basis: one CLI route expands a graph root and materializes the resulting skill set.
...
- Total: 19
- Split decision: score 16-24 retained because all responsibilities serve one tree checkout route.

## Source Field Carryover

- Source purpose:
  - Materialize all skills resolved from a graph root into a project active surface in stable order.
...
- `checkout tree` command behavior
- stable materialization plan for graph roots
- selected root lockfile recording

Does not own:

- single-skill checkout internals except reuse
- status/update command behavior
- overlay or dirty recovery
...
- Parent responsibilities owned by this child:
  - tree/root checkout materialization, stable order, and selected-root lockfile recording.
...
|---|---:|---|---|---|
| `project/spec-refinement-history/2.0.0p-20260717-190429.md` | 2 | `project/specs/2.0.0p/checkout-tree-materialization.spec.md` | `none` | reached |

## Implementation Routing

- Primary modules/files:
  - `skills_keeper/checkout.py` - tree checkout planning/materialization
...
- Reusable library/module files:
  - `skills_keeper/checkout.py` - tree checkout planning/materialization
- Tests:
  - `tests/test_checkout.py` - tree checkout CLI tests

## Chosen Defaults / Parameters

- copy order follows traversal result order
- partial failure rolls back or preserves explicit failure evidence
- selected roots are recorded in lockfile

## Data Ownership

- Source of truth: graph root traversal and library source store.
- Read ownership: checkout tree command.
- Write ownership: checkout tree command writes active copies and lockfile.
- Derived/cache data: active copies derive from library source.
...
- Background/concurrency route, if applicable:
  - not applicable; watcher may observe completed active-folder writes after the command returns.
...
- Progression handling:
  - implement after traversal and lockfile schema.
...
- DTOs/models:
  - `CheckoutPlan` - root id and materialization entries
  - `CheckoutResult` - materialized paths and warnings
- Functions/methods:
  - `checkout_tree(workspace, root_id) -> CheckoutResult`
  - `plan_checkout_tree(manifest, root_id) -> CheckoutPlan`
...
- Tree checkout materializes all resolved skills.
- Lockfile records selected root and each entry.
- Output lists materialized skills in stable order.
- Failure does not silently leave untracked generated copies.
