...
Count: 1 IWU.
Basis: one narrow archive command fix that removes a single computed generated mirror after successful managed global archive.
...
- Total: 20
- Split decision: score 16-24 retained because path helper, deletion, and command output are one bug-fix route and splitting would separate unshippable halves.

## Source Field Carryover

- Source purpose:
  - Remove the generated Codex mirror when a managed global skill is archived.
...
- Source split/provenance notes:
  - Reviewed from the original draft during review pass 1; no split was required.
...
- generated mirror path helper
- mirror deletion after successful global archive
- archive command output for mirror status

Does not own:

- checkout reconciliation
- fully managed local behavior
- source folder deletion policy
...
|---|---:|---|---|---|
| `project/spec-refinement-history/2.0.0p-20260717-190429.md` | 2 | `project/specs/2.0.0p/global-archive-mirror-removal.spec.md` | `none` | reached |

## Implementation Routing

- Primary modules/files:
  - `skills_keeper/cli.py` - archive command integration
...
- Reusable library/module files:
  - `skills_keeper/cli.py` - archive command integration
- Tests:
  - `tests/test_cli.py` - archive CLI regression tests

## Chosen Defaults / Parameters

- default prefix remains `keld`
- missing mirror is success with `missing` status
- delete only one computed path under configured root

## Data Ownership

- Source of truth: datastore active/archive state and generated Codex mirror root.
- Read ownership: archive command.
- Write ownership: archive command moves datastore active copy and deletes computed mirror.
- Derived/cache data: mirror path derives from identity, prefix, and root.
...
- Background/concurrency route, if applicable:
  - not applicable; archive mirror removal is an explicit command route.
...
- Progression handling:
  - can implement independently after review.
...
- Observable result: datastore archive path, generated mirror path, removed/missing/skipped mirror status, stdout, and exit code.
...
- DTOs/models:
  - `ArchiveResult` - archive path, mirror path, mirror status
  - `MirrorRemovalStatus` - removed, missing, skipped
- Functions/methods:
  - `generated_codex_mirror_path(identity, prefix, root) -> Path`
  - `remove_generated_codex_mirror(identity, prefix, root) -> MirrorRemovalStatus`
  - `archive_managed_global_skill(...) -> ArchiveResult`
...
- Managed global archive removes existing generated mirror.
- Missing mirror reports missing and succeeds.
- Custom prefix removes only matching path.
- Unrelated mirrors remain.
- Backup-only local archive behavior is unchanged.
